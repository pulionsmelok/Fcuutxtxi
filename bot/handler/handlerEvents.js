...
module.exports = function (api, threadModel, userModel, dashBoardModel, globalModel, usersData, threadsData, dashBoardData, globalData) {
	...
	return async function (event, message) {
		...
		async function onStart() {
			...
			let prefixMode = GoatBot.config.prefixModeEnabled === true;
			try {
				const fs = require("fs");
				const modeFile = `${process.cwd()}/scripts/cmds/S1DD1K/prefixmode.json`;
				if (fs.existsSync(modeFile)) prefixMode = JSON.parse(fs.readFileSync(modeFile, "utf8"))?.enabled === true;
			} catch (_) {}
